
{
     ...
     * 1. 安全性。any类型基本上是对类型检查的一种放弃。unknown类型则是一种安全的any。它表示一个值的类型未知，因此TypeScript不允许
     * 我们对unknown类型的变量进行任意操作。如果我们想要对一个unknown类型的变量进行操作，我们必须首先进行类型检查或类型断言，以确保操作的安全性。
     * 2. 使用场景。使用any可以快速地编写代码，不受类型系统的限制，但这通常不推荐，因为这样做会失去TypeScript提供的大部分类型安全保障。
     * unknown类型适用于那些我们确实不知道或不关心具体类型的情况，但我们仍然希望在编译时保持类型安全。例如，处理外部数据或编写类型安全的通用函数时，
     * unknown是一个更好的选择。
     * 3. 类型兼容性：any类型与任何其他类型互相兼容，这意味着你可以将any类型的值赋给任何类型的变量，反之亦然。
