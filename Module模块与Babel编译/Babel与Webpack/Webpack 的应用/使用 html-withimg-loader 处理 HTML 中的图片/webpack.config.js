...
也能保证在最终构建的项目中资源引用的正确性。
 ...
 总的理解起来，就是为什么需要使用html-withimg-loader，其实就是为了配合webpack的模块化打包，将图片资源也打包进去。
 在构建过程中，Webpack 会读取 HTML 文件及其中的图像引用，并使用此 loader 来处理这些引用。进行优化/压缩/改位置/生成图片哈希
 等操作。并自动更新图片引用，以确保最终的构建结果中所有图像都能正确加载。如果这个图片没有找到，在编译时会报错。
 */